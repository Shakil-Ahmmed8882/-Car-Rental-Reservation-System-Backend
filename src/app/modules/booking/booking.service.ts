...
const BookCarIntoDB = async (email:string, payload: TBooking) => {
    ...
    if(car.status !== 'available'){
      throw new AppError(httpStatus.NOT_FOUND,'Opps! This Car is not available!')
    }
    
    // check is the car already booked by same user
    const isBookedBySameUser = await BookingModel.findOne({user:user._id,car:car._id}).select('_id')
  ...
  const result =  await BookingModel.create(payload);
    ...
    const populatedResult = await BookingModel.findById(result._id).populate('user').populate('car');
